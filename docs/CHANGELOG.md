...
- Fixed Server Inventory collection cards so primary operational content retains full card width and `Connect`, `Edit`, and `Delete` actions reflow below it under constrained width and 200% font scale, while preserving lazy rendering and stable item identity without typography or pagination changes.
